  ...
  mount(el, user) {
    ...
    const brief = () => {
      ...
      return [
        ...
        `TRADING — THE LIVE MIND v1.3, leak-audited and frozen 28 August 2026. Verified BACKTEST (never live money): $100,000 to $3,427,531 over June 2016 to March 2026, 1,845 trades, 59.7% win rate, profit factor 1.50, max drawdown 19.9%, CAGR 43.9%. Three session legs: ES morning ex-Friday, NQ morning, NQ afternoon sniper. Two earlier look-ahead leaks were found in an internal audit, removed, and are now blocked by an automated leak guard — this record is the honest one.`,
        `SIM GATE: 0 of 60 verified simulated trades. ZERO recorded fills. If asked how the bot performs live, the only honest answer is that it has not traded yet.`,
        `EXECUTION STATUS: the live engine is BUILT (oracle pattern — the verified backtester itself is the live brain) with a graduation ladder for any account size from $1k up. Broker applications (IBKR + Optimus) in progress; launch plan is shadow-live first, then broker paper, then live at half size after 60 in-band trades.`,
