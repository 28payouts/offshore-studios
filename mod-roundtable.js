  ...
  mount(el, user) {
    ...
    const brief = () => {
      ...
      return [
        ...
        `TRADING — the engine set has been frozen since 27 July 2026. Verified BACKTEST only, recomputed from the raw trade files: $100,000 to $10,015,774 across January 2008 to June 2026 on one-minute data, 6,164 trades, 56.2% win rate, profit factor 1.66, worst drawdown 16.9%, legs on ES and NQ. This is NOT live money and has never been live money.`,
        `SIM GATE: 0 of 60 verified simulated trades. There are ZERO recorded fills. If asked how the bot is performing live, the only honest answer is that it has not traded.`,
        `CRITICAL GAP: the verified v6 engine set exists as a PYTHON simulation, not as Pine Script. It therefore cannot run on TradingView and cannot fire alerts as things stand. The Pine files in the project (APlus_ICT_v4 and older) are a DIFFERENT, earlier strategy with its own separate results. Nothing can be recorded live until this gap is closed — either by porting v6 to Pine or by running the Python live against a broker feed.`,
