...
OS.register({
  ...
  mount(el, user) {
    ...
    const LAW = " ABSOLUTE RULES: the $100k→$10,015,774 figure is a VERIFIED BACKTEST over 18 years, never live money — always say backtest. There are ZERO live fills; the sim gate stands at 0/60 and must pass before real capital. Never invent a number, a fill, a date or a client. If you do not know, say so.";
    ...
    const ctx = () => {
      ...
      const base = {
        ...
        record: "$100k→$10,015,774 backtested over 18y · 6,164 trades · 56.2% win rate · PF 1.66 · max drawdown 16.9% · rules frozen 27 Jul 2026 · sim gate 0/60 · zero live fills",
        engines: "four engines: E1 foundation ES (PF 1.53), E2 narrative NQ (1.68), E3 crossover NQ (1.96), E4 mirror ES (1.50)",
    ...
    function localBrain(q) {
      ...
      if (SCOPE === "principal") {
        if (s.includes("bot") || s.includes("mind") || s.includes("status") || s.includes("stand"))
          return `The Live Mind is ${c.mind.toLowerCase()} as of ${c.ny}. The rules have been frozen since 27 July 2026 and the verified backtest stands at $100k→$10.02M over eighteen years — 6,164 trades, 56.2% win rate, profit factor 1.66, worst drawdown 16.9%. No live fills yet: the sim gate is at 0 of 60. [go:livemind]`;
        if (s.includes("record") || s.includes("prove") || s.includes("real"))
          return `What it proves: the logic survived eighteen years of real one-minute data bar-by-bar, including 2020 and 2022, with a 1.66 profit factor. What it does not prove: live execution — no slippage, no real fills, no live spread. That gap is exactly what the 0/60 sim gate exists to close before any capital is committed.`;
        if (s.includes("live") || s.includes("money") || s.includes("before"))
          return `Three steps, in order. First the TradingView webhook connects so trades are logged automatically. Then sixty verified simulated trades are compared against the 56.2% record. Only if that holds does the smallest possible live size go in, with the rules unchanged.`;
      ...
      if (SCOPE === "trading") {
        ...
        if (s.includes("record") || s.includes("result") || s.includes("perform"))
          return `The verified backtest covers eighteen years: 6,164 trades, 56.2% winners, profit factor 1.66, worst drawdown 16.9%. Important: that is historical testing, not live trading results. Live performance is expected to run below backtest.`;
      ...
      if (s.includes("how") && (s.includes("bot") || s.includes("mind") || s.includes("doing"))) return `She's ${c.mind.toLowerCase().split(" (")[0]} right now — ${c.ny}. Frozen record: $10.02M backtested, 56.2% win, gate at 0/60. Nothing moves until the gate earns it. [go:livemind]`;
